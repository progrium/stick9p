...
#if defined(BH_PLATFORM_STICK)
/* AppCpu worker stack is small; keep WASI init pointers off the stack frame. */
static struct fd_table *stick_wasi_curfds;
static struct fd_prestats *stick_wasi_prestats;
                       ...
                       char *error_buf, uint32 error_buf_size)
{
    ...
    char *path;
#if defined(BH_PLATFORM_STICK)
    char resolved_path[STICK_WASI_RESOLVED_PATH_BYTES];
#else
...
#if defined(BH_PLATFORM_STICK)
    stick_wasi_curfds = curfds;
    stick_wasi_prestats = prestats;
#endif
...
#if defined(BH_PLATFORM_STICK)
    curfds = stick_wasi_curfds;
    if (!curfds) {
        set_error_buf(error_buf, error_buf_size,
                      "Init wasi environment failed: fd table missing");
        goto fail;
    }
#endif
...
#if defined(BH_PLATFORM_STICK)
    wasi_ctx->curfds = stick_wasi_curfds;
    wasi_ctx->prestats = stick_wasi_prestats;
    stick_wasi_curfds = NULL;
    stick_wasi_prestats = NULL;
#else
