...
8. Wire `/dev/mic/pcm` and `/dev/spk/pcm` with DMA driving Embassy channels. **Status (StickS3):** `/dev/spk/{ctl,pcm,info}` is live. The audio task runs `I2s::write_dma_circular_async` against a **6144-byte** stereo ring (≈ 96 ms @ 16 kHz) allocated by the `dma_circular_buffers!` macro — that macro lays the buffer out as a word-aligned `static mut [u32; …]` and builds the correct descriptor count for circular mode (3 descriptors for a buffer ≤ 2 × chunk_size). A prior attempt that `Box::leak`-ed a `Vec<u8>` and called `write_dma_circular_async(&*buf)` wedged the chip on this board because heap allocations are byte-aligned and DMA circular reads at 64 KiB/s require word alignment; the macro fixes that root cause. **Pitfall:** for circular buffers where `len ≤ chunk_size * 2`, esp-hal sizes each of the 3 descriptors as `len/3 + len%3` (first descriptor) and `len/3` (remaining two). With a 4 KiB ring that came out as 1366/1365/1365 — none divisible by 4. `push_with` then advances `write_offset` by those values, so the slices it hands back start mid-stereo-frame, every L/R sample gets cut at byte offset 2, and the DAC plays a loud metallic high-frequency aliasing tone. The ring size must be a **multiple of 12** so all three descriptors stay on 4-byte (stereo frame) boundaries. The 9P side accepts mono s16le into a 32 KiB ring; `push_with` drains up to 512-byte mono chunks per refill, applies a Q8 software gain, expands mono→stereo (L=R) directly into the DMA slice, and pads any remainder with silence so BCLK/LRCK stay clocked continuously (no tremolo, no click between chunks). Underruns (running with empty ring) increment `under=` in `cat /dev/spk/ctl`. Mic capture is still deferred; the codec is already configured for either direction.
