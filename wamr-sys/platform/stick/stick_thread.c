/*
 * pthread-based threading for Stick / esp-hal (adapted from WAMR esp-idf port).
                           ...
                           unsigned int stack_size, int prio)
{
    ...
    if (pthread_attr_setstacksize(&tattr, stack_size) != 0) {
        os_printf("Invalid thread stack size %u. Min stack size = %u\n",
                  stack_size, (unsigned)PTHREAD_STACK_MIN);
...
{
    return pthread_join(thread, retval) == 0 ? BHT_OK : BHT_ERROR;
}

int
os_thread_detach(korp_tid tid)
{
    return pthread_detach(tid) == 0 ? BHT_OK : BHT_ERROR;
}
...
{
    return pthread_cond_wait(cond, mutex) == 0 ? BHT_OK : BHT_ERROR;
}
...
{
    return pthread_cond_signal(cond) == 0 ? BHT_OK : BHT_ERROR;
}

int
os_cond_broadcast(korp_cond *cond)
{
    return pthread_cond_broadcast(cond) == 0 ? BHT_OK : BHT_ERROR;
}
...
{
    assert(lock);
    return pthread_rwlock_init(lock, NULL) == 0 ? BHT_OK : BHT_ERROR;
}
...
{
    assert(lock);
    return pthread_rwlock_rdlock(lock) == 0 ? BHT_OK : BHT_ERROR;
}
...
{
    assert(lock);
    return pthread_rwlock_wrlock(lock) == 0 ? BHT_OK : BHT_ERROR;
}
...
{
    assert(lock);
    return pthread_rwlock_unlock(lock) == 0 ? BHT_OK : BHT_ERROR;
}
...
{
    assert(lock);
    return pthread_rwlock_destroy(lock) == 0 ? BHT_OK : BHT_ERROR;
}
